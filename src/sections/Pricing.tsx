...
const Pricing = () => {
  ...
  <div className="bg-white">
    <h2 className='text-center font-bold tracking-tighter p-5 bg-gradient-to-b md:text-[54px] md:leading-[60px] from-black to-indigo-800 text-transparent bg-clip-text text-3xl max-w-[540px] mx-auto'>
      Pricing
    </h2>
    ...
    <div className="flex flex-col gap-6 items-center mt-10 lg:flex-row lg:justify-center">
      {pricingTiers.map(({ title, monthlyPrice, buttonText, popular, inverse, features }, index) => (
        <div key={index} className={twMerge("p-10 border max-w-xs w-full border-[#F1F1F1] rounded-3xl shadow-[0_7px_14px_#EAEAEA]", inverse === true && 'border-black bg-black text-white')}>
          ...
          <button className={twMerge("bg-black text-white w-full px-4 mt-[30px] py-2 p-6 rounded-lg font-medium inline-flex align-items justify-center tracking-tight", inverse === true && "text-black bg-white")}>
            {buttonText}
          </button>
